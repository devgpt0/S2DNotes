# JavaScript - Beginner to Expert Roadmap

JavaScript adds behavior to web pages and also runs outside browsers. These notes focus on modern browser JavaScript using modules and strict, maintainable practices.

## Chapters

0. [JavaScript concepts in simple words](00a-javascript-concepts-in-simple-words.md)
1. [Runtime, variables, values, and types](01-runtime-variables-values-and-types.md)
2. [Operators, equality, coercion, and control flow](02-operators-equality-coercion-and-control-flow.md)
3. [Functions, scope, closures, and `this`](03-functions-scope-closures-and-this.md)
4. [Arrays, objects, Map, Set, JSON, and immutability](04-arrays-objects-map-set-json-and-immutability.md)
5. [DOM, events, forms, and accessible interaction](05-dom-events-forms-and-accessibility.md)
6. [Event loop, promises, async/await, Fetch, and cancellation](06-event-loop-promises-async-await-fetch.md)
7. [Errors, modules, and code organization](07-errors-modules-and-code-organization.md)
8. [Prototypes, classes, iterators, and generators](08-prototypes-classes-iterators-and-generators.md)
9. [Modern and advanced language concepts](09-modern-and-advanced-language-concepts.md)
10. [Browser APIs, storage, security, observers, and workers](10-browser-apis-storage-security-observers-and-workers.md)
11. [Performance, memory, testing, and debugging](11-performance-memory-testing-and-debugging.md)
12. [Practical activities and interview answers](12-javascript-practical-and-interview-answers.md)
13. [Expert JavaScript tips and browser patterns](98-javascript-expert-tips.md)
14. [60-minute task board project](99-javascript-60-minute-project.md)

## First Rule

Do not manipulate a page until you understand variables, functions, arrays/objects, and errors. Do not start a framework until you understand the DOM, events, modules, and asynchronous JavaScript.
