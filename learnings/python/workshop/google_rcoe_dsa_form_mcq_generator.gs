...
function createPbcoeVacAssessmentNo1() {
  const config = {
    title: "RCOE VAC Assessment DSA Quiz",
    description: [
      ...
      "â€¢ Submit the form only after answering all questions.",
  ...
  const bootcodingInterestItem = form.addMultipleChoiceItem();
  bootcodingInterestItem
    .setTitle("How many of you are interested in knowing about Bootcoding Courses.")
    .setChoices(
      ["Very interested", "Interested", "Not sure", "Not interested"].map(
        (option) => bootcodingInterestItem.createChoice(option),
      ),
    )
    .setRequired(true);

  form.addPageBreakItem().setTitle("DSA Lists MCQ Test");
...
function validateQuestionBank(questions) {
  if (questions.length !== 60) {
    throw new Error("Question bank must contain 60 questions; found " + questions.length);
  }
  ...
  questions.forEach((question, index) => {
    ...
    question.options.forEach((option) => {
      ...
      if (optionTexts.has(normalizedOption)) {
        throw new Error("Question " + (index + 1) + " has a repeated option");
      }
  ...
  const theoryQuestionCount = questions.length - codeQuestionCount;
  if (theoryQuestionCount !== 15 || codeQuestionCount !== 45) {
    throw new Error("Question bank must contain 15 theory and 45 code questions");
  }
}
...
function verifyGeneratedQuestions(form, questions, config) {
  ...
  if (items.length !== questions.length + 1) {
    throw new Error("Generated " + items.length + " MCQs; expected " + (questions.length + 1));
  }

  const interestItem = items[0];
  if (interestItem.getTitle() !== "How many of you are interested in knowing about Bootcoding Courses.") {
    throw new Error("The Bootcoding interest question was not saved correctly");
  }
  if (interestItem.getPoints() !== 0) {
    throw new Error("The Bootcoding interest question must not be scored");
  }

  let totalPoints = 0;
  items.slice(1).forEach((item, index) => {
    const [questionTitle, code] = questions[index].question.split("\n\n");
    if (item.getTitle() !== String(index + 1) + ". " + questionTitle) {
    ...
    if (item.getPoints() !== pointsForQuestion(index, config)) {
  ...
  if (totalPoints !== config.totalPoints) {
    throw new Error("Quiz has incorrect total points: " + totalPoints);
  }
}
...
function buildQuestionBank() {
  ...
  const theoryQuestions = questions
    .filter((question) => !question.question.includes("\n\n"))
    .slice(0, 15);
  const codeQuestions = questions
    .filter((question) => question.question.includes("\n\n"))
    .slice(0, 45);
  return [...theoryQuestions, ...codeQuestions];
}
